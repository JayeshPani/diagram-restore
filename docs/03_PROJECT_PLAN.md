...
| 9. Generalization | Evaluate held-out corruption severity and layout density (per `docs/04_EXPERIMENT_PROTOCOL.md` §9); a second synthetic renderer or independently authored diagrams remain future work. | `results/milestone4/STAGE9_GENERALIZATION_REPORT.md` | **Done for two shifts.** Models trained once per seed (3 seeds) on confirmatory-v1, evaluated zero-shot against 2x corruption severity and always-5-node density. U0/U1 dominance over D0/D1 holds in every condition (gap-to-noise ratio ~6.6x-30x); no ranking flip. Renderer/acquisition shift not yet tested — flagged as the one remaining generalization category. |
...
| 8 (done) | Generalization: 2x corruption severity and always-5-node density shifts, 3 seeds (Stage 9) | `results/milestone4/STAGE9_GENERALIZATION_REPORT.md` — dominance holds in every condition |
...
| Only the pilot generator shows gains | Limit the claim to that domain and strengthen independent validation before a broader paper — **addressed**: Stage 9 confirms the gap survives two distribution shifts (harder corruption, denser layouts); renderer/acquisition shift remains untested. |
