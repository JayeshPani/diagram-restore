...
| 9. Generalization | Evaluate held-out corruption severity, layout density, and a second synthetic renderer (per `docs/04_EXPERIMENT_PROTOCOL.md` §9); independently authored diagrams remain future work needing external input. | `results/milestone4/STAGE9_GENERALIZATION_REPORT.md`, `results/milestone4/STAGE9_RENDERER_SHIFT_REPORT.md` | **Done for three shifts.** Models trained once per seed (3 seeds) on confirmatory-v1, evaluated zero-shot against 2x corruption severity, always-5-node density, and a second renderer (`geometry.render_antialiased`, a genuinely different rasterization technique). U0/U1 dominance over D0/D1 holds in every condition and *strengthens* under density and renderer shift (U-Net reaches near-ceiling; diffusion's variance rises 14x under the renderer shift specifically). Only the acquisition-shift category (real/independently authored diagrams) remains — needs external human input this project cannot fabricate. |
...
| 12. Literature refresh | Before submission, redo the closest-work search (Stage 2) specifically for diagram/graphics-recognition **benchmark and evaluation** papers, not routing/efficient-diffusion papers — the positioning target has changed. | Updated `docs/02_LITERATURE_REVIEW.md` and `docs/07_SEARCH_LOG.md`. | **Two passes done** (web search + full-text follow-up, evidence labels A/B): SciFlow-Bench (arXiv:2602.09809) read in full text — a near-concurrent, independently-mechanized argument for structural-recoverability-over-visual-similarity in diagram *generation*, which also independently found diffusion trailing on structural fidelity (a different task, same qualitative pattern). OPRB's venue/authors verified (DocRevive, CVPR 2026 Workshop MULA). A systematic citation-graph check and full-text reads of OPRB/SciFlow-Bench's Table 4 remain before drafting. |
...
| 8 (done) | Generalization: 2x corruption severity, always-5-node density, and a second renderer, 3 seeds each (Stage 9) | `results/milestone4/STAGE9_GENERALIZATION_REPORT.md`, `STAGE9_RENDERER_SHIFT_REPORT.md` — dominance holds and strengthens in every condition |
| 9 (partial) | Literature refresh for benchmark framing (Stage 12, two passes done), draft manuscript (Stage 10) | Updated lit review; complete results section and manuscript draft |
...
| Only the pilot generator shows gains | Limit the claim to that domain and strengthen independent validation before a broader paper — **addressed**: Stage 9 confirms the gap survives and strengthens under three distribution shifts (harder corruption, denser layouts, a second renderer); only an acquisition shift (real diagrams) remains untested, pending external input. |
