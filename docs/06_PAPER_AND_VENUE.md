...
| The result generalizes to a genuine distribution shift | Held-out corruption/density/renderer shifts, and (resources permitting) independently created examples | **Done for corruption, density, and renderer, 3 seeds each** — gap-to-noise ratio ~3.9×–30×, no ranking flip, strengthens under density/renderer shift; `results/milestone4/STAGE9_GENERALIZATION_REPORT.md`, `STAGE9_RENDERER_SHIFT_REPORT.md`. Only acquisition shift (real diagrams) still open, needing external input. |
